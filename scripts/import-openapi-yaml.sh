...
#   - multipart/form-data binary fields: contentMediaType = "application/octet-stream" -> format = "binary"
#
...
if [ $? -ne 0 ]; then
  echo "Failed to patch OpenAPI YAML"
  exit 1
fi

poetry run python -m yq -e '
  [
    .components.schemas.Body_create_code_scorer_version_scorers__scorer_id__version_code_post.properties.file.format == "binary",
    (.components.schemas.Body_create_code_scorer_version_scorers__scorer_id__version_code_post.properties.file | has("contentMediaType") | not),
    .components.schemas.Body_create_dataset_datasets_post.properties.file.anyOf[0].format == "binary",
    (.components.schemas.Body_create_dataset_datasets_post.properties.file.anyOf[0] | has("contentMediaType") | not),
    .components.schemas.Body_manual_llm_validate_multipart_scorers_llm_validate_multipart_post.properties.query_files.items.format == "binary",
    (.components.schemas.Body_manual_llm_validate_multipart_scorers_llm_validate_multipart_post.properties.query_files.items | has("contentMediaType") | not),
    .components.schemas.Body_manual_llm_validate_multipart_scorers_llm_validate_multipart_post.properties.response_files.items.format == "binary",
    (.components.schemas.Body_manual_llm_validate_multipart_scorers_llm_validate_multipart_post.properties.response_files.items | has("contentMediaType") | not),
    .components.schemas.Body_validate_code_scorer_dataset_scorers_code_validate_dataset_post.properties.file.format == "binary",
    (.components.schemas.Body_validate_code_scorer_dataset_scorers_code_validate_dataset_post.properties.file | has("contentMediaType") | not),
    .components.schemas.Body_validate_code_scorer_log_record_scorers_code_validate_log_record_post.properties.file.format == "binary",
    (.components.schemas.Body_validate_code_scorer_log_record_scorers_code_validate_log_record_post.properties.file | has("contentMediaType") | not),
    .components.schemas.Body_validate_code_scorer_scorers_code_validate_post.properties.file.format == "binary",
    (.components.schemas.Body_validate_code_scorer_scorers_code_validate_post.properties.file | has("contentMediaType") | not)
  ] | all
' "$HOME_DIR/openapi.yaml" > /dev/null

if [ $? -ne 0 ]; then
  echo "Failed to patch multipart binary fields in OpenAPI YAML"
  exit 1
fi
