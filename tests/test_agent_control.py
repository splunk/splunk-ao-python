...
def _stub_cached_logger(monkeypatch, logger: SimpleNamespace) -> None:
    key = (threading.current_thread().name, "batch", logger.project_name, logger.agent_stream_name)
...
def test_get_agent_control_target_uses_explicit_log_stream_id() -> None:
    ...
    target = get_agent_control_target(agent_stream_id=agent_stream_id)

    # Then: the target is log-stream scoped
    assert target == AgentControlTarget(target_type="log_stream", target_id=agent_stream_id)
...
def test_get_agent_control_target_uses_env_project_id_with_explicit_log_stream_id(monkeypatch) -> None:
    ...
    assert target == AgentControlTarget(target_type="log_stream", target_id=agent_stream_id, project_id=project_id)
...
def test_get_agent_control_target_uses_env_log_stream_id(monkeypatch) -> None:
    ...
    agent_stream_id = str(uuid4())
    project_id = str(uuid4())
    monkeypatch.setenv("SPLUNK_AO_AGENT_STREAM_ID", agent_stream_id)
    monkeypatch.setenv("SPLUNK_AO_PROJECT_ID", project_id)
...
def test_get_agent_control_target_strips_env_ids(monkeypatch) -> None:
    ...
    agent_stream_id = str(uuid4())
    project_id = str(uuid4())
    monkeypatch.setenv("SPLUNK_AO_AGENT_STREAM_ID", f"  {agent_stream_id}  ")
...
def test_get_agent_control_target_strips_explicit_log_stream_target_id() -> None:
    ...
    # When: resolving an Agent Control target
    target = get_agent_control_target(target_id=f"  {agent_stream_id}  ")
...
def test_get_agent_control_target_uses_cached_context_logger(monkeypatch) -> None:
    ...
    # When: resolving an Agent Control target
    with splunk_ao_context(project="project-a", agent_stream="stream-a"):
        target = get_agent_control_target()
...
def test_get_agent_control_target_uses_cached_default_logger(monkeypatch) -> None:
    ...
    logger = SimpleNamespace(
        ...
        agent_stream_name=DEFAULT_AGENT_STREAM_NAME,
        project_id=project_id,
        agent_stream_id=agent_stream_id,
        terminate=lambda: None,
    )
...
def test_get_agent_control_target_ignores_cached_logger_from_other_thread(monkeypatch) -> None:
    ...
    _stub_cached_loggers(
        monkeypatch, {("other-thread", "batch", DEFAULT_PROJECT_NAME, DEFAULT_AGENT_STREAM_NAME): logger}
    )
...
def test_get_agent_control_target_prefers_cached_logger_project_id_over_env(monkeypatch) -> None:
    ...
    assert target == AgentControlTarget(target_type="log_stream", target_id=agent_stream_id, project_id=cached_project_id)
...
def test_get_agent_control_target_uses_explicit_project_id_with_cached_logger(monkeypatch) -> None:
    ...
    assert target == AgentControlTarget(
        target_type="log_stream", target_id=agent_stream_id, project_id=explicit_project_id
    )
...
def test_get_agent_control_target_rejects_invalid_explicit_log_stream_id() -> None:
    ...
    # When/Then: resolving the target fails before sending a malformed target to Agent Control
    with pytest.raises(AgentControlTargetUnresolvedError, match="agent_stream_id='prod' is not a valid UUID"):
        get_agent_control_target(agent_stream_id="prod")
...
def test_get_agent_control_target_rejects_conflicting_ids() -> None:
    ...
    with pytest.raises(AgentControlTargetUnresolvedError, match="target_id and agent_stream_id must match"):
        get_agent_control_target(target_id=str(uuid4()), agent_stream_id=str(uuid4()))
