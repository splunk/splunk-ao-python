...
class TestGalileoOTLPExporter:
    ...
    def mock_config(self):
        ...
        with patch("splunk_ao.otel.GalileoPythonConfig.get") as mock_config_get:
            config = Mock()
...
class TestGalileoSpanProcessor:
    ...
    def mock_processor_setup(self):
        ...
        with (
            ...
            patch("splunk_ao.otel.GalileoOTLPExporter") as mock_exporter_class,
        ):
    ...
    @pytest.mark.skipif(not OTEL_AVAILABLE, reason="OpenTelemetry not available")
    @patch("splunk_ao.otel.GalileoOTLPExporter")
...
class TestOTelIntegration:
    ...
    @patch("splunk_ao.otel.OTLPSpanExporter.__init__", return_value=None)
    @patch("splunk_ao.otel.GalileoPythonConfig.get")
...
class TestOTelContextIntegration:
    ...
    def mock_processor_deps(self):
        ...
        with (
            ...
            patch("splunk_ao.otel.GalileoOTLPExporter") as mock_exp,
        ):
    ...
    def mock_exporter(self):
        ...
        with (
            patch("splunk_ao.otel.OTLPSpanExporter.__init__", return_value=None),
            patch("splunk_ao.otel.GalileoPythonConfig.get") as mock_config_get,
        ):
            config = Mock()
            config.api_url = "https://api.galileo.ai"
            config.api_key = SecretStr("test-key")
