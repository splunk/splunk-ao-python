...
_os.environ["GALILEO_CONSOLE_URL"] = "http://fake.test:8088"
...
def _fast_config_validation() -> Generator[None, None, None]:
    """HYBIM-790: building GalileoPythonConfig runs 3 async validation requests
    ...
    (see .local/HYBIM-790-investigation.md). They are already mocked, so they add
    no coverage — only event-loop cost. Replace them with canned, await-free
    results so the dispatch is trivial. Scoped to the per-test config build only;
    test bodies still exercise the real validation/connect code."""
...
def set_validated_config(
    ...
    # HYBIM-790: bypass the slow async validation round-trips for the build only.
