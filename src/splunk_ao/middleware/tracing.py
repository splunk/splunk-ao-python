...
class TracingMiddleware(BaseHTTPMiddleware):
    ...
    (SPLUNK_AO_PROJECT and SPLUNK_AO_AGENT_STREAM). They are not propagated via headers,
...
def get_request_logger() -> SplunkAOLogger:
    ...
    (SPLUNK_AO_PROJECT and SPLUNK_AO_AGENT_STREAM), not propagated via headers, following
    standard distributed tracing patterns.

    If no tracing headers were present in the request, a regular logger is returned
    (using SPLUNK_AO_PROJECT and SPLUNK_AO_AGENT_STREAM env vars).
    ...
    # Project and log_stream come from env vars (SPLUNK_AO_PROJECT, SPLUNK_AO_AGENT_STREAM)
