...
class StarConstraints:
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        d = dict(src_dict)
        annotation_type = cast(Literal["star"], d.pop("annotation_type"))
        if annotation_type != "star":
            raise ValueError(f"annotation_type must match const 'star', got '{annotation_type}'")

        star_constraints = cls(annotation_type=annotation_type)
