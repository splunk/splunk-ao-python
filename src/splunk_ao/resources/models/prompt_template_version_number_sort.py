...
T = TypeVar("T", bound="PromptTemplateVersionNumberSort")
...
class PromptTemplateVersionNumberSort:
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        d = dict(src_dict)
        name = cast(Union[Literal["version"], Unset], d.pop("name", UNSET))
        if name != "version" and not isinstance(name, Unset):
            raise ValueError(f"name must match const 'version', got '{name}'")
        ...
        prompt_template_version_number_sort = cls(name=name, ascending=ascending, sort_type=sort_type)

        prompt_template_version_number_sort.additional_properties = d
        return prompt_template_version_number_sort
