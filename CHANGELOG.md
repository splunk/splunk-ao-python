...
- **`OpenAIGalileo` renamed to `OpenAISplunkAO`** (HYBIM-726): The OpenAI wrapper class in
  `splunk_ao.openai` has been renamed as part of the Galileo → Splunk AO rebrand. Any code
  directly importing this class by name must update to
  `from splunk_ao.openai import OpenAISplunkAO`.
